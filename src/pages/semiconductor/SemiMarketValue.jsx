    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <div className="flex items-center gap-2 text-emerald-300">
              ...
              <span className="text-sm font-semibold">Market Value Intelligence</span>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg border border-border bg-surface px-3 py-2 text-text transition-colors hover:border-emerald-400/60 hover:text-emerald-300"
            >
      ...
      {(marketData.hasFallback || error) && (
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-200">
          {error || '일부 금융 데이터 연결에 실패해 해당 기업은 샘플 가격 흐름으로 표시 중입니다.'}
        </div>
      )}
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-sm text-textMuted">선택한 지역 기준으로 7개 산업군의 시장가치와 평균 등락률을 비교합니다.</p>
        </div>
            ...
            return (
              <div key={segment.id} className="rounded-xl border border-border bg-surface p-4">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={`rounded-lg px-2 py-1 text-xs font-bold ${isUp ? 'bg-emerald-500/15 text-emerald-300' : 'bg-red-500/15 text-red-300'}`}>
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-lg border px-2.5 py-1.5 text-xs transition-colors ${
                  selectedIds.includes(company.companyId)
                    ? 'border-emerald-400/60 bg-emerald-500/15 text-text'
                    : 'border-border text-textMuted hover:text-text'
                }`}
              >
            ...
            {rankedCompanies.map((company) => {
              ...
              return (
                <button
                  key={company.companyId}
                  onClick={() => toggleCompany(company.companyId)}
                  className="w-full rounded-xl border border-border bg-background p-3 text-left transition-colors hover:border-emerald-400/50"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-1.5">
                        ...
                        {rankDelta !== 0 && (
                          <span className={`text-[10px] ${rankDelta > 0 ? 'text-emerald-300' : 'text-red-300'}`}>
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <p className={`mt-1 inline-flex items-center justify-end gap-1 text-xs font-semibold ${isUp ? 'text-emerald-300' : 'text-red-300'}`}>
          ...
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                />
                <Area type="monotone" dataKey="value" stroke="#22c55e" fill="rgba(34, 197, 94, 0.18)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
